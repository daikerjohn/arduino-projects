...
EpeverTracer::EpeverTracer()
{
    ...
    this->_timer.setInterval(this->_delayTime);
}
...
bool EpeverTracer::update()
{
    ...
    if (!this->_timer.isReady()) {
        //TelnetPrint.println("MODBUS update() timer.");
        return true;
    }
    ...
    {
      TelnetPrint.println("MODBUS update() result != ku8MBSuccess.");
        return false;
    }
    //TelnetPrint.println("MODBUS update() fallthrough");
    return true;
}
